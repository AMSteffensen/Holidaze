...
export default () => {
    ...
    const getEstablishment = (id) => establishments.find(({ id: eId }) => eId === id)?.name || id;
        ...
        <div className="w-full overflow-auto">
            <table className="table-auto">
                ...
                <tbody className="text-gray-800">
                    {
                        enquiries.map(({ id, name, email, establishmentId, checkIn, checkOut }) => <tr key={id}>
                            <td className="px-4 py-2">{name}</td>
                            <td className="px-4 py-2">{email}</td>
                            {
                                establishmentId !== getEstablishment(establishmentId) ?
                                <Link to={`/hotels/${id}`}>
                                    <td className="px-4 py-2 underline font-semibold">{getEstablishment(establishmentId)}</td>
                                </Link>
                                :
                            ...
                            <td className="px-4 py-2"><button className="" onClick={() => deleteEnquiry(id)}><i className="fa fa-trash"></i></button></td>
                        </tr>)
                    }
                </tbody>
            </table>
